...
- **Clickable Citation Links in RFP Responses** (September 30): Fixed issue where Pinecone's clickable source links were not appearing in RFP responses. Updated the source structure from string arrays to objects containing both name and URL properties. Modified both backend (`RFPResponse` interface) and frontend (`RFPResponseTab` component) to preserve and display clickable links when URLs are available. Sources now render as proper hyperlinks that open in new tabs, matching Pinecone's direct output behavior.
...
- **RFP Response Parsing Fix** (September 30): Fixed RFP response parsing issue where removing all formatting instructions caused Pinecone to return unstructured responses that the parser couldn't handle, resulting in cascading duplicate answers. Added minimal formatting instructions (just response structure, no persona) to ensure proper parsing while preserving Pinecone's configured behavior.
