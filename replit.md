...
- **RFP Text Extraction Spacing Fix** (September 23): Fixed issue where RFP questions displayed with excessive character-level spacing (like "w o r d s   l i k e   t h i s"). The text extraction function now properly detects and fixes character-level spacing patterns while preserving normal word boundaries. This ensures questions are readable and AI processing is more accurate.
